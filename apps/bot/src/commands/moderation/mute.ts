/**
 * `mute` - timeout a member for a duration (mute = native Discord timeout).
 */
...
export class MuteCommand extends Command {
  public constructor(context: Command.LoaderContext, options: Command.Options) {
    super(context, { ...options, name: 'mute', description: 'Mute (timeout) a member for a duration.' })
  }
  ...
  private async run(
    ...
    await applyTimeout(ctx, member, durationMs, reason, 'mute')
  }
}
