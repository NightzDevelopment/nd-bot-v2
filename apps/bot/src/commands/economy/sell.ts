...
export class SellCommand extends Command {
    ...
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addIntegerOption((opt) =>
          opt.setName('quantity').setDescription('How many to sell.').setRequired(false).setMinValue(1),
        ),
    )
  }
  ...
  public override async messageRun(message: Message, args: Args): Promise<void> {
    ...
    if (!key) {
      await message.reply({ embeds: [errorEmbed('Sell', t(locale, 'economy.shop.not_found'))] })
      return
    }
  ...
  private async build(guildId: string, userId: string, key: string, qty: number) {
    ...
    if (!item) {
      return errorEmbed('Sell', t(locale, 'economy.shop.not_found'))
    }
    const result = await service.sell(guildId, userId, item, qty)
    if (!result.ok) {
      return errorEmbed('Sell', t(locale, 'common.invalid_amount'))
    }
    const label = qty > 1 ? `${qty}x ${item.name}` : item.name
    return successEmbed(
      'Sell',
      t(locale, 'economy.shop.sold', { item: label, amount: money(locale, result.refund) }),
