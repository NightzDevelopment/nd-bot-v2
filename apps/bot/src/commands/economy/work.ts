...
export class WorkCommand extends Command {
  public constructor(context: Command.LoaderContext, options: Command.Options) {
    super(context, {
      ...options,
      name: 'work',
      description: 'Work a shift to earn some coins.',
    })
  }
  ...
  private async build(guildId: string, userId: string) {
    ...
    const result = await economyService().work(guildId, userId)
    if (!result.ok) {
      return warningEmbed('Work', t(locale, 'economy.work.cooldown', { time: formatDuration(result.retryInMs) }))
    }
    return successEmbed('Work', t(locale, 'economy.work.success', { amount: money(locale, result.amount) }))
